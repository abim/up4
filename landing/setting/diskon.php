...
//=========================================================================//
	...
	if($_POST['tambahsubmit']){
		...
		$row = $authresult = $tambah -> proses($_POST['jenis'], $_POST['jumlah']);
		if($row[0] == "kosong1"){
...
//=========================================================================//
	...
	if($_POST['update']){
		$tanggal = date("Y-m-d H:i:s");
		$sql -> db_Update("info_diskon", "
				`potongan`='".$_POST['jumlah']."', 
...
//=========================================================================//
	if($_POST['hapus']){
		...
		$sql -> db_Delete("info_diskon", "`jenis`='".$_POST['jenis']."' ");
...
//=========================================================================//
	...
	if(c_QUERY == "ksg1"){
		$tambah -> judul("JENIS NAMA TIDAK TERISI DENGAN BENAR, ULANGI PENGISIAN", 1);
		$tambah -> formulir();
	}
	if(c_QUERY == "ksg2"){
		$tambah -> judul("JUMLAH TIDAK TERISI, ULANGI PENGISIAN", 1, 1);
		$tambah -> formulir();
	}
	if(c_QUERY == "goup"){
...
function edit($jenis) {
		...
		$sql -> db_Select("info_diskon", "*", "jenis='{$jenis}'");
		$result = $sql -> db_Fetch(); extract($result);
$teks =  "

<h2>Pengubahan ".$jenis."</h2>

<form method='post' action='". c_SELF ."'>
". $form->buka("post", c_SELF) . $form->hidden("jenis", $jenis) ."
<div class=\"form-wrap\">

	<div class=\"form_col_1\">
		<label for=\"jenis\">Jenis Diskon</label><em>*</em>
	</div>
	<div class=\"form_col_2\">
		". $form -> text("jenis", "", $jenis,"","","","","",'readonly') ."
	</div>	
	<div class=\"clear\"></div><br/>
	
	<div class=\"form_col_1\">
		<label for=\"jumlah\">Jumlah Diskon</label><em>*</em>
	</div>
	<div class=\"form_col_2\">
		". $form -> text("jumlah", 3, $potongan) ." %
		<p>contoh: 20</p>
	</div>	
	<div class=\"clear\"></div><br/>
...
function hapus($jenis) {
		...
		$teks =  "APAKAH ANDA YAKIN AKAN MENGHAPUS DATA <span style='color:red;'><em>".$jenis."</em></span>?
";
		$button = "
...
<h2>Record Diskon</h2>";
	...
	$sql -> db_Select("info_diskon", "*", "jenis!='0' ORDER BY jenis ASC LIMIT ".$halaman.",".ITEMVIEW."");
	$sql2 -> db_Select("info_diskon");
	...
	<thead>
		...
		<th scope='col' class=\"num tgl\">Jumlah Diskon</th>
	...
	while($row = $sql-> db_Fetch()){
		...
		$teks .= "
		<tr {$jav}>
			<td class='uppercase'>". $row['jenis'] ."
			<div class=\"row-actions\">
					<span class='edit'><a href=\"". c_SELF ."?edit.".$row['jenis']."\">Edit</a> | </span>
					<span class='delete'><a class='delete' href='". c_SELF ."?hapus.".$row['jenis']."'>Delete</a></span>
				</div>
			</td>
			<td class='num'>". $row['potongan'] ." %</td>
